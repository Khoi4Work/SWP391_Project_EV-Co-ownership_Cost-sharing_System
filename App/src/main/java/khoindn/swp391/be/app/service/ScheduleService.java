...
public class ScheduleService implements IScheduleService {
    ...
    public void updateSchedule(ScheduleReq req, int scheduleId) {
        ...
        boolean hasCheckedIn = iCheckInRepository.existsBySchedule_ScheduleId(scheduleId);
        boolean hasCheckedOut = iCheckOutRepository.existsBySchedule_ScheduleId(scheduleId);

        if (hasCheckedIn || hasCheckedOut) {
            throw new InvalidOperationException(
                    "Không thể cập nhật schedule này vì xe đã được check-in hoặc check-out."
            );
        }
        validateTimeRange(req.getStartTime(), req.getEndTime());
    ...
    public void cancelSchedule(int scheduleId) {
        ...
        if (hasCheckedIn || hasCheckedOut) {
            throw new InvalidOperationException(
                    "Không thể hủy schedule này vì xe đã được check-in hoặc check-out."
            );
        }
    ...
    private void checkTimeConflicts(int groupId, LocalDateTime startTime, LocalDateTime endTime,
        ...
        // Check cross-group conflicts first
        List<StatusSchedule> excludedStatuses = List.of(
                StatusSchedule.CANCELED,
                StatusSchedule.OVERRIDDEN,
                StatusSchedule.OVERRIDE_TRACKER
        );

        List<Schedule> userGlobalConflicts = iScheduleRepository
                .findByGroupMember_Users_IdAndStatusNotIn(user.getId(), excludedStatuses)
                .stream()
                .filter(s -> excludeScheduleId == null || s.getScheduleId() != excludeScheduleId)
                .filter(s -> s.getStartTime().isBefore(endTime) && s.getEndTime().isAfter(startTime))
                .toList();

        if (!userGlobalConflicts.isEmpty()) {
            String conflictDetails = userGlobalConflicts.stream()
                    .map(s -> String.format("Nhóm '%s': %s - %s",
                            s.getGroupMember().getGroup().getGroupName(),
                            s.getStartTime().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")),
                            s.getEndTime().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm"))))
                    .collect(Collectors.joining(" | "));

            throw new TimeConflictException(
                    String.format("Không thể đặt lịch. Bạn đã có %d lịch trùng giờ ở nhóm khác: %s",
                            userGlobalConflicts.size(), conflictDetails)
            );
        }
