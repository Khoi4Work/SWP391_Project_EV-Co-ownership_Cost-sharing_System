            ...
            <Card className="shadow-sm">
                <CardContent className="pt-6">
                    <div className="grid gap-4 md:grid-cols-3 items-end">
                        ...
                        <div>
                            <div className="mb-3">
                                <div className="text-sm text-muted-foreground mb-2">Đóng góp hàng tháng của bạn theo tỉ
                                    lệ sử dụng:
                                </div>
                                {me && <div className="flex items-center justify-between text-xs">
                                    <span>{me.name}: {getUserUsage(me.id)}%</span>
                                    <span
                                        className="font-medium text-primary">{getMonthlyContribution(me.id).toLocaleString()} VNĐ</span>
                                </div>}
                            </div>

                            <Input id="amount" type="number" min={min} placeholder={`${min}`} value={amount}
                        ...
                        <Tabs defaultValue="vehicles">
                            ...
                            <TabsContent value="vehicles">
                                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                    {group.vehicles.map(v => <div key={v.id}
                                                                  className={`border rounded-md p-3 cursor-pointer transition-colors ${selectedVehicle === v.id ? "border-primary bg-primary/5" : "hover:bg-accent/50"}`}
                                                                  onClick={() => setSelectedVehicle(v.id)}>
                                        <img src={v.imageUrl} alt={`Hình ảnh xe ${v.name}`} loading="lazy"
                                        ...
                                        <Badge
                                            variant={v.status === "maintenance" ? "destructive" : v.status === "in-use" ? "secondary" : "default"}>
                            ...
                            <TabsContent value="requests">
                                <div className="space-y-3">
                                    {serviceRequests.length === 0 ?
                                        ...
                                        </div> : serviceRequests.map(request => <div key={request.id}
                                                                                     className="border rounded-lg p-4">
                                            <div className="flex items-start justify-between">
                                                <div className="flex-1">
                                                    <div className="font-medium">{request.vehicleName}</div>
                                                    <div
                                                        className="text-sm text-muted-foreground">{request.serviceType}</div>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <Textarea value={serviceDescription} onChange={e => setServiceDescription(e.target.value)}
                                  placeholder="Nhập mô tả chi tiết về dịch vụ cần thực hiện..." className="mt-1"
                                  rows={3}/>
                    </div>
